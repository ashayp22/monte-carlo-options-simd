...
fn criterion_benchmark1(c: &mut Criterion) {
    ...
    c.bench_function("monte carlo fast", |b| b.iter(|| {
        for spot in START_SPOT..END_SPOT {
            _ = mcfast::call_price(spot as f32, STRIKE, VOLATILITY, RISK_FREE_RATE, YEARS_TO_EXPIRY, DIVIDEND_YIELD, NUM_STEPS, NUM_TRIALS, &mut rng);
        }
    }));
    ...
    c.bench_function("monte carlo 32x8", |b| b.iter(|| {
        for i in (START_SPOT..END_SPOT).step_by(8) {
            ...
            _ = mc32x8::call_price(spot, strike_f32x8, volatility_f32x8, risk_free_rate_f32x8, years_to_expiry_f32x8, dividend_yield_f32x8, NUM_STEPS, NUM_TRIALS, &mut rng);
        }
    }));
}
